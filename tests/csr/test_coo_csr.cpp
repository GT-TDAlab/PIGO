...
int back_forth_sptr(string dir_path) {
    COO<uint32_t, uint32_t, shared_ptr<uint32_t>> coo { dir_path + "/ba_100_14_1.el", EDGE_LIST };
    ...
    auto x1 = coo.x().get(); auto y1 = coo.y().get();
    auto x2 = coo2.x(); auto y2 = coo2.y();
    ...
    for (size_t i = 0; i < coo.m(); ++i) {
        EQ(coo.x().get()[idx1[i]], coo2.x()[idx2[i]]);
        EQ(coo.y().get()[idx1[i]], coo2.y()[idx2[i]]);
    }
